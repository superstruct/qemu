...
function findWasmBinary() {
    return locateFile('gio-querymodules.wasm');
}
...
// Begin JS library code
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(6, 0, 1, fd, length);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(7, 0, 1, buf, size);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(8, 0, 1, fd, dirp, count);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(14, 0, 1, fds, nfds, timeout);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(15, 0, 1, olddirfd, oldpath, newdirfd, newpath);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(16, 0, 1, path, buf);
