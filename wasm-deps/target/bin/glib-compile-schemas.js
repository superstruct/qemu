...
function findWasmBinary() {
    return locateFile('glib-compile-schemas.wasm');
}
...
// Begin JS library code
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(3, 0, 1, fd, buf);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(4, 0, 1, fd, length);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(5, 0, 1, fd, dirp, count);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(10, 0, 1, olddirfd, oldpath, newdirfd, newpath);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(12, 0, 1, dirfd, path, flags);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(13, 0, 1, len, prot, flags, fd, offset, allocated, addr);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(14, 0, 1, addr, len, prot, flags, fd, offset);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(17, 0, 1, fd);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(18, 0, 1, fd, pbuf);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(20, 0, 1, fd, offset, whence, newOffset);
