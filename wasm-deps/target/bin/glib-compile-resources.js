...
function findWasmBinary() {
    return locateFile('glib-compile-resources.wasm');
}
...
// Begin JS library code
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(8, 0, 1, fd, length);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(16, 0, 1, olddirfd, oldpath, newdirfd, newpath);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(17, 0, 1, path, buf);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(19, 0, 1, len, prot, flags, fd, offset, allocated, addr);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(21, 0, 1, __environ, environ_buf);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(22, 0, 1, penviron_count, penviron_buf_size);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(23, 0, 1, fd);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(24, 0, 1, fd, pbuf);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(26, 0, 1, fd, offset, whence, newOffset);
  ...
  function(fd) {
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(27, 0, 1, fd);
