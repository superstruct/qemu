...
function findWasmBinary() {
    return locateFile('gi-decompile-typelib.wasm');
}
...
// Begin JS library code
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(12, 0, 1, fdPtr);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(14, 0, 1, path, buf);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(15, 0, 1, len, prot, flags, fd, offset, allocated, addr);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(16, 0, 1, addr, len, prot, flags, fd, offset);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(19, 0, 1, fd);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(20, 0, 1, fd, pbuf);
